package com.flow.snesde.uilib.animatedGifLabel;
 ...
 * {@link AnimatedGifLabel} SWT class (see
 * {@link #setGifImageFromPluginResource(String)}</b><br />
 * <b>Changes by Sorceror, (a)sync.exec(...) call fix by YAMaiDie</b>
 * </p>
 */
public class AnimatedGifLabel extends Canvas
{
	...
	public AnimatedGifLabel(Composite parent, int style)
	{
	...
	private void initAccessible()
	{
		...
		accessible.addAccessibleListener(new AccessibleAdapter()
		{
			...
			{
				char mnemonic = _findMnemonic(AnimatedGifLabel.this.text);
				if (mnemonic != '\0')
				{
	...
	/**
	 * Enables setting of a plugin resource as gif image input
	 * 
	 * @param path
	 *          should be of the following format:
	 *          <pluginName>/path/morepath/yourgif.gif. Example:
	 *          com.flow.snesde.filecomputil/icons/loading.gif
	 */
	public void setGifImageFromPluginResource(String path)
	{
		URL url;
		InputStream inputStream = null;
		try
		{
			url = new URL("platform:/plugin/" + path);
		...
		this.setGifImage(inputStream);
	}

	// private void setGifImage(String path)
	// {
	// try
	// {
	...
	private class GifThread extends Thread
	{
		...
		public void run()
		{
			while (run)
			{
				...
				if (!AnimatedGifLabel.this.isDisposed())
				{
					AnimatedGifLabel.this.getDisplay().asyncExec(new Runnable()
					{
						@Override
						public void run()
						{
							...
							if (!AnimatedGifLabel.this.isDisposed())
							{
								imageNumber = imageNumber == loader.data.length - 1 ? 0
										: imageNumber + 1;
								if (!AnimatedGifLabel.this.image.isDisposed())
									AnimatedGifLabel.this.image.dispose();
								ImageData nextFrameData = loader.data[imageNumber];
								AnimatedGifLabel.this.image = new Image(AnimatedGifLabel.this
										.getDisplay(), nextFrameData);
								AnimatedGifLabel.this.redraw();
							}
							else stopRunning();
						}
					});
